...
document.querySelector<HTMLElement>("[data-year]")?.replaceChildren(
  document.createTextNode(String(new Date().getFullYear())),
);

const fileInput = document.querySelector<HTMLInputElement>("[data-files]");
const fileList = document.querySelector<HTMLElement>("[data-file-list]");
const photoError = document.querySelector<HTMLElement>('[data-error="photos"]');
...
form?.addEventListener("submit", (event) => {
  ...
  if (firstInvalid) {
    firstInvalid.focus();
    if (status) status.textContent = "Проверьте заполнение формы.";
    return;
  }
  ...
  if (status) status.textContent = "Ничего не отправлено: это демонстрационная форма.";
});
